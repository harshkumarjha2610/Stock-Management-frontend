...
async function refreshAccessToken(): Promise<string | null> {
  try {
    ...
    // ✅ Adjust these keys based on what your refresh API returns
...
async function fetchWithAuth(
  ...
  if (response.status === 401) {
    console.warn('⚠️ Got 401 — trying token refresh...');
...
// ─── AllocationChart with API + Console Logs ─────────────
  ...
  useEffect(() => {
    const fetchWalletBalance = async () => {
      try {
        ...
        console.log('✅ State updated — chart should now show API data');
      } catch (err) {
          ...
          {loading ? (
            ...
            <>
              <div className="text-xl lg:text-2xl font-bold text-white">
                {fetched && totalAmount > 0 ? formattedTotal : '$35,450'}
              </div>
              <div className="text-white/60 text-[10px] lg:text-xs">
                {fetched && totalAmount > 0 ? 'Simulation Wallet' : 'Divided Wallet'}
              </div>
            </>
          )}
        </div>
