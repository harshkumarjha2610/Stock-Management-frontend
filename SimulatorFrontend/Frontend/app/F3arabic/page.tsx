...
// ============ NAVIGATION DATA (Arabic) ============
const navigationItems = [
  { label: "لوحة المستثمر", href: "#" },
  { label: "المحفظة", href: "#" },
  { label: "المجتمع", href: "#" },
];
...
export default function SimulatorDashboardF3(): JSX.Element {
  ...
  return (
    <div 
      dir="rtl" 
      className="bg-black w-full min-h-screen flex flex-col px-4 md:px-8 lg:px-12"
    ...
    >
      ...
      <header className="w-full max-w-[1836px] h-auto md:h-[97px] self-center mt-4 md:mt-[25px] bg-[#3D3D3D] rounded-[20px] md:rounded-[30px] border border-white/10 relative px-4 py-4 md:py-0">
        {/* Logo - RTL positioned */}
        <div className="flex items-center justify-center md:absolute md:top-5 md:right-[25px] mb-4 md:mb-0">
          <img
        ...
        <nav className="flex flex-col md:flex-row items-center gap-2 md:gap-2.5 md:absolute md:top-[22px] md:left-1/2 md:transform md:-translate-x-1/2">
          {navigationItems.map((item, index) => (
            ...
            >
              <span className="font-normal text-white text-base md:text-xl tracking-[0] leading-[normal]">
                {item.label}
              </span>
            </Button>
          ))}
        ...
        {/* Right side image - RTL positioned (left side) */}
        <div className="hidden md:flex md:absolute md:top-[22px] md:left-[15px] lg:left-[25px] items-center justify-center">
          <img
      ...
      {/* Project title - RTL layout */}
      <div className="flex flex-row-reverse items-center gap-3 md:gap-5 mt-4 md:mt-[23px]">
        <img
        ...
        />
        <h1 className="font-bold text-white text-lg sm:text-xl md:text-2xl lg:text-3xl tracking-[0] leading-tight">
          مشروع XYZ
        </h1>
      </div>
      ...
      <Card className="w-full max-w-[1829px] min-h-[500px] sm:min-h-[600px] md:min-h-[700px] lg:h-[800px] xl:h-[846px] mt-6 md:mt-9 mb-6 rounded-[15px] bg-[#3D3D3D] border border-white/10 relative overflow-hidden">
        <CardContent className="p-0 h-full w-full relative">
          ...
          <div className="absolute bottom-[3%] sm:bottom-[4%] md:bottom-[5%] lg:bottom-[6%] xl:bottom-[8%] left-0 right-0 flex items-end justify-center gap-3 sm:gap-6 md:gap-8 lg:gap-12 xl:gap-20 px-2 sm:px-4 md:px-6 lg:px-8 z-[1]">
            ...
            {/* RIGHT: Tall building (mirrored position for RTL) - Slightly reduced for larger screens */}
            <img
            ...
            {/* LEFT: Phase 3 text + House + Progress bar (mirrored to right for RTL) */}
            <div className="flex flex-col items-center gap-2 sm:gap-3 md:gap-4 mb-[-20px] sm:mb-[-25px] md:mb-[-30px] lg:mb-[-40px] xl:mb-[-50px]">
              ...
              <div className="flex flex-col items-center gap-1 md:gap-2 mb-2 md:mb-3">
                <div className="font-bold text-white text-sm md:text-lg lg:text-xl xl:text-[27px] text-center drop-shadow-lg">
                  المرحلة 3
                </div>
                <div className="font-medium text-white text-base md:text-xl lg:text-2xl xl:text-[32.3px] drop-shadow-lg">
                  مكتملة
                </div>
              </div>
              ...
              {/* Progress bar - dynamic (RTL: filling from right to left) */}
              <div className="w-[110px] sm:w-[140px] md:w-[180px] lg:w-[250px] xl:w-[320px] 2xl:w-[380px] h-[20px] sm:h-[24px] md:h-[28px] lg:h-[36px] xl:h-[42px] rounded-full bg-[#3D3D3D] border border-white/20 overflow-hidden shadow-lg">
                <div className="relative w-full h-full">
                  <div
                    className="absolute top-0 right-0 h-full bg-[#ef6b23] rounded-full flex items-center justify-start pl-1 sm:pl-2 md:pl-3 transition-all duration-300 ease-out"
                    style={{ width: `${progress}%` }}
                  >
                    <span className="font-bold text-white text-[10px] sm:text-xs md:text-sm lg:text-base xl:text-lg whitespace-nowrap">
                      {progress}٪
                    </span>
                  </div>
                </div>
